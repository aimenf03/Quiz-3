...
public class QuickSort<T extends Comparable<T>> {
    ...
    public static <T extends Comparable<T>> T[] sort(PIVOT_TYPE pivotType, T[] unsorted) {
        if (unsorted == null || unsorted.length == 0) {
            return unsorted;  // return empty or null arrays
        }
    ...
    private static <T extends Comparable<T>> void sort(int index, int start, int finish, T[] unsorted) {
        ...
        if (start < f) {
            int newPivot = getRandom((f - start) + 1);
            sort(newPivot, start, f, unsorted);
        }
        if (s < finish) {
            int newPivot = getRandom((finish - s) + 1);
            sort(newPivot, s, finish, unsorted);
        }
    }
